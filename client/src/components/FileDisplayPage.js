...
const FileDisplayPage = () => {
    const [files, setFiles] = useState([])
     ...
     useEffect(() => {
      fetch(`/upload/upload`).then(res => res.json()).then(data => {
          setFiles(data); console.log('files PARENT component is -------->', data)
    ...
    return (
        ...
        <p>File display here</p>
            ...
            <div>{files.map((f, i) => (<div> <PDFViewer document={{url: f }} /> {console.log('indiv file PARENT component is', f)} <Link to={`/file/display/singlefile/${i}`} > Link Here</Link> </div>))}</div>
            {/* <div>{files.map((f) => (<div> <img src={f} />  </div>))}</div> */}
            {/* <>{files.map((f, i) => (<div><Link to= {`/singlefile/${i}`} /></div>))}</> */}
